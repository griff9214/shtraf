...
class JsonResponseDecoder implements ResponseDecoder
{
    ...
    public static function getSecureCode($response)
    {
        // TODO: Implement getSecureCode() method.
    }
...
function secureCodeRequest(Car $car)
{
    ...
    if (!JsonResponseDecoder::checkErrors($response)) {
        //$secureKey = json_decode($response, true)['key'];
        $secureKey = "906bd6add30ec7eabcda22985ef6123a";
    } else {
...
function photoRequest(Fine $fine)
{
    $client = new Client();
...
var_dump(JsonResponseDecoder::getFines($finesResponse));
?>